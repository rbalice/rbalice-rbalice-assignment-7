...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    for _ in range(S):
        ...
        X_sim = X_sim = np.random.rand(N)  # Replace with code to generate simulated X values
        Y_sim = intercept + slope * X_sim + np.random.normal(loc=mu, scale=np.sqrt(sigma2), size=N)  # Replace with code to generate simulated Y values

        # TODO 7: Fit linear regression to simulated data and store slope and intercept
        sim_model =  LinearRegression().fit(X, Y_sim)  # Replace with code to fit the model
    ...
    slope_more_extreme = np.mean(np.abs(slopes) > abs(slope))  # Replace with code to calculate proportion of slopes more extreme than observed
    intercept_extreme = np.mean(np.abs(intercepts) > abs(intercept))  # Replace with code to calculate proportion of intercepts more extreme than observed
...
def hypothesis_test():
    ...
    print(f"Form data - parameter: {parameter}, test_type: {test_type}")
    ...
    # p_value = None
    print(f"Test type selected: {test_type}")
    print(f"Observed statistic: {observed_stat}")
    print(f"Hypothesized value: {hypothesized_value}")

    if test_type == ">":
        p_value = np.mean(simulated_stats >= observed_stat)
        print(f"Greater than test p-value: {p_value}")
    elif test_type == "<":
        p_value = np.mean(simulated_stats <= observed_stat)
        print(f"Less than test p-value: {p_value}")
    else:
        p_value = np.mean(np.abs(simulated_stats - hypothesized_value) >= abs(observed_stat - hypothesized_value))
        print(f"Two-sided test p-value: {p_value}")

    # TODO 11: If p_value is very small (e.g., <= 0.0001), set fun_message to a fun message
    fun_message = "The result is highly significant!" if p_value <= 0.0001 else None

    # TODO 12: Plot histogram of simulated statistics
    plt.figure()
    plt.hist(simulated_stats, bins=20, color="skyblue", alpha=0.5, label="Simulate Statistics")
    plt.axvline(observed_stat, color="red", linestyle="--", label=f"Observed {parameter}: {observed_stat:.4f}")
    plt.axvline(hypothesized_value, color="blue", linestyle="-", label=f"Hypothesized {parameter} (H₀):{hypothesized_value}")
...
def confidence_interval():
    ...
    line_color = "green" if includes_true else "red"
    ...
    plt.figure()
    plt.scatter(estimates, np.zeros_like(estimates), color="grey", alpha=0.5, label="Simulated Estimates")
    plt.plot(mean_estimate, 0, 'o', color="blue", label="Mean Estimate")
    plt.hlines(0, ci_lower, ci_upper, color="blue", linewidth=4, label=f"{confidence_level}% Confidence Interval")
    plt.axvline(true_param, color=line_color, linestyle="--", label="True Slope")
